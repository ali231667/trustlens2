...
def fetch_instagram_profile(username: str) -> dict:
    ...
    profile_data = {
        ...
        # Full-resolution (~1080px) picture for the AI-image detector — the plain
        # profile_pic_url is only a 150x150 thumbnail, too small for a verdict.
        "profile_pic_url_hd": (data.get("hd_profile_pic_url_info") or {}).get("url", "") or "",
...
def fetch_engagement_data(username: str, amount: int = 12) -> dict:
    ...
    comments = []
    post_codes = []
    reel_codes = []   # video posts only — usable by the transcriber
    ...
    for edge in edges:
        ...
        if code:
            ...
            # media_type 2 == video/reel; is_video is set on some responses
            if node.get("media_type") == 2 or node.get("is_video"):
                reel_codes.append(code)
